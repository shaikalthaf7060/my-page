...
export function setCharTimeline(character, camera) {
  ...
  let screenLight = null, monitor = null, monitorMeshList = [];
        ...
        if (object.material) {
          if (Array.isArray(object.material)) {
            object.material.forEach((m) => {
              m.transparent = true;
              m.opacity = 0;
              if (m.name === "Material.018") m.color.set("#FFFFFF");
            });
          } else {
  ...
  if (window.innerWidth > 1024) {
    if (character) {
      ...
      if (monitor) {
        if (monitor.material) {
          if (Array.isArray(monitor.material)) {
            monitor.material.forEach((mat) => {
        ...
        monitorMeshList.forEach((mesh) => {
          if (mesh !== monitor && mesh.material) {
            tl2.to(mesh.material, { opacity: 1, duration: 0.8, delay: 3.2 }, 0);
          }
        });
        tl2.fromTo(
          monitor.position,
